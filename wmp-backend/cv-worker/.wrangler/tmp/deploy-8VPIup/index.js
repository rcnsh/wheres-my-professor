...
async function graphql(env, query) {
  const url = env.WEAVIATE_CLUSTER_URL.startsWith("http") ? env.WEAVIATE_CLUSTER_URL : `https://${env.WEAVIATE_CLUSTER_URL}`;
...
async function extractEmbedding(env, imageBlob) {
...
__name(extractEmbedding, "extractEmbedding");
async function searchByBase64(env, base64Image, options = {}) {
  ...
  try {
    ...
    const objects = data?.Get?.FaceEmbedding ?? [];
    const personMatches = /* @__PURE__ */ new Map();
    for (const item of objects) {
      const personName = item.personName;
      const distance = item._additional?.distance ?? 1;
      if (threshold !== void 0 && distance > threshold) continue;
    ...
    const allMatches = Array.from(personMatches.values()).sort((a, b) => a.distance - b.distance).slice(0, topK);
    return {
...
__name(searchByBase64, "searchByBase64");
async function listPeople(env, limit = 100) {
  const data = await graphql(
    env,
  ...
  return Array.from(peopleMap.entries()).map(([personName, photoCount]) => ({ personName, photoCount })).slice(0, limit);
}
__name(listPeople, "listPeople");
async function getStats(env) {
  const data = await graphql(
    env,
    `{ Aggregate { FaceEmbedding { meta { count } } } }`
  );
  const totalEmbeddings = data?.Aggregate?.FaceEmbedding?.[0]?.meta?.count ?? 0;
  const people = await listPeople(env, 1e4);
...
app.post("/analyse", async (c) => {
  try {
    ...
    const rawEmotions = await hfResponse.json();
    const emotions = rawEmotions.map((obj) => {
